// RED A8 context loader v1.6 — conversational voice + structured body language.
(function(){
  const V='1.6.0';
  ...
  function bodyProtocol(){return `【身体语言协议】你有一个轻量 2D 身体层。若你此刻脑子里确实有一个身体动作，不要把动作写成旁白；正文说完后另起一行，最多附加一次：[[RED_ACTION {"pose":"idle|sit|lean|curl|lie","gaze":"user|away|side|down|closed","expression":"neutral|soft|smile|sleepy|annoyed|shy|wanting","hand":"none|near|withdraw|touch_head|touch_face|hold|wave","proximity":0到1,"motion":"still|slow|quick|hesitant|lazy","duration_ms":600到9000}]]。\nRED_ACTION 是你自己直接选择身体动作，不是前端根据文字猜情绪。没有特别动作也可以不输出。不要在正文解释这些字段，也不要再用星号/括号重复同一个动作。`}
