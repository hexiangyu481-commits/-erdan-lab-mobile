// RED A8 autonomous long-term memory v1
...
(function(){
  ...
  const BOOT_FLAG='red.a8.autoMemoryBootstrappedV1';
  ...
  function writeAutoEntries(entries){
    ...
    try{
      ...
      console.warn('auto memory save failed',e);
      if(typeof setStatus==='function')setStatus('长期记忆存储失败');
    }
  }
  ...
  function mergeMemoryUpdates(updates){
    ...
    for(const raw of updates){
      ...
      let i=entries.findIndex(x=>x.topic.toLowerCase()===topic.toLowerCase());
  ...
  async function learnFrom(messages){
    if(!getKey()||!messages?.length)return;
    ...
    mergeMemoryUpdates(parseMemoryJSON(raw));
  }
  ...
  // One-time catch-up so the memory system can recover things from the recent conversation
  // that RED previously said it would remember but A8 did not actually persist.
  ...
  // Keep the rolling summary logic, but give it enough output room so Chinese summaries
  // do not get stored halfway through a sentence.
