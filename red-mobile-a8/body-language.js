// R Avatar Frame Motion v3.0 — persistent expression + one-shot micro actions chosen by R.
(function(){
  const V='3.0.0';
  ...
  const def={pose:'idle',gaze:'user',expression:'soft',hand:'none',proximity:.48,motion:'slow',micro:'none',durationMs:3200,updatedAt:0};
  let current=null,activeTimer=null,blinkTimer=null,microTimer=null,observer=null,lastPlayedStamp=0;
...
#ravMotion,#rAvatarPortrait,#ravHead,#ravEyes,#ravPupils,#ravMouth,#ravHand,#ravShoulders{transform-box:fill-box;transform-origin:center;transition:transform var(--rav-speed,1s) cubic-bezier(.2,.72,.2,1),opacity .5s ease}
...
#rAvatarFrame.gaze-closed .rav-eye,#rAvatarFrame.blink .rav-eye,#rAvatarFrame.micro-close_eyes .rav-eye{transform:scaleY(.08);transform-origin:center}
#rAvatarFrame.gaze-closed #ravPupils,#rAvatarFrame.blink #ravPupils,#rAvatarFrame.micro-close_eyes #ravPupils{opacity:0}
...
#rAvatarFrame.exp-smile #ravMouth,#rAvatarFrame.micro-smile #ravMouth{transform:scaleX(1.18) translateY(-.3px)}
...
#rAvatarFrame.micro-turn #ravMotion{animation:ravTurn 1.15s cubic-bezier(.32,.05,.2,1) 1}
#rAvatarFrame.micro-nod #ravMotion{animation:ravNod .72s ease-in-out 1}
#rAvatarFrame.micro-shake #ravMotion{animation:ravShake .72s ease-in-out 1}
#rAvatarFrame.micro-bounce #ravMotion{animation:ravBounce .72s ease-out 1}
#rAvatarFrame.micro-lean_in #ravMotion{animation:ravLeanIn .95s ease-in-out 1}
#rAvatarFrame.micro-pull_back #ravMotion{animation:ravPullBack .95s ease-in-out 1}
#rAvatarFrame.micro-glance #ravPupils{animation:ravGlance .9s ease-in-out 1}
#rAvatarFrame.micro-look_down #ravPupils{animation:ravLookDown .9s ease-in-out 1}
#rAvatarFrame.micro-blink .rav-eye{animation:ravBlink .34s ease-in-out 1;transform-origin:center}
#rAvatarFrame.micro-blink #ravPupils{animation:ravPupilBlink .34s ease-in-out 1}
...
@keyframes ravTurn{0%{transform:rotate(0) scale(1)}45%{transform:rotate(185deg) scale(.91)}100%{transform:rotate(360deg) scale(1)}}
@keyframes ravNod{0%,100%{transform:translateY(0) rotate(0)}42%{transform:translateY(2px) rotate(3deg)}70%{transform:translateY(-.5px) rotate(-1deg)}}
@keyframes ravShake{0%,100%{transform:translateX(0)}25%{transform:translateX(-2.2px) rotate(-2deg)}55%{transform:translateX(2.2px) rotate(2deg)}78%{transform:translateX(-1px)}}
@keyframes ravBounce{0%,100%{transform:translateY(0)}35%{transform:translateY(-3px) scale(1.02)}65%{transform:translateY(1px)}}
@keyframes ravLeanIn{0%,100%{transform:scale(1)}45%,70%{transform:scale(1.13) translateY(1px)}}
@keyframes ravPullBack{0%,100%{transform:scale(1)}45%,70%{transform:scale(.88) translateY(-1px)}}
@keyframes ravGlance{0%,100%{transform:translateX(0)}35%,70%{transform:translateX(2.2px)}}
@keyframes ravLookDown{0%,100%{transform:translateY(0)}35%,70%{transform:translateY(2px)}}
@keyframes ravBlink{0%,100%{transform:scaleY(1)}50%{transform:scaleY(.06)}}
@keyframes ravPupilBlink{0%,100%{opacity:1}50%{opacity:0}}
@media (prefers-reduced-motion:reduce){#rAvatarFrame,#rAvatarFrame *{transition:none!important;animation:none!important}}
`;
    document.head.appendChild(style);
    const frame=document.createElement('span');frame.id='rAvatarFrame';frame.setAttribute('aria-hidden','true');frame.innerHTML=`<svg viewBox="0 0 48 48" role="presentation"><g id="ravMotion"><g id="rAvatarPortrait"><path class="rav-hair" d="M10 25C9 10 16 5 24 5c10 0 16 7 15 20 0 7 2 11 4 15-5-1-8-4-10-7-6 5-13 5-20 0-2 3-5 6-8 7 3-6 5-10 5-15Z"/><path class="rav-face" d="M15 22c0-9 3-14 9-14 7 0 10 6 10 14 0 9-4 15-10 15-6 0-9-6-9-15Z"/><g id="ravHead"><g id="ravEyes"><ellipse class="rav-eye" cx="20" cy="21" rx="2.5" ry="1.65"/><ellipse class="rav-eye" cx="29" cy="21" rx="2.5" ry="1.65"/><g id="ravPupils"><circle class="rav-pupil" cx="20.4" cy="21" r=".9"/><circle class="rav-pupil" cx="29.4" cy="21" r=".9"/></g></g><ellipse class="rav-blush" cx="17.6" cy="26" rx="2.4" ry="1"/><ellipse class="rav-blush" cx="31.7" cy="26" rx="2.4" ry="1"/><path id="ravMouth" class="rav-line" d="M21.5 29q2.5 1.8 5 0"/></g><g id="ravShoulders"><path class="rav-line" d="M16 37c-4 2-6 5-7 9M32 37c4 2 6 5 7 9M16 37q8 6 16 0"/></g><g id="ravHand"><path class="rav-line" d="M35 37c4 0 5-3 4-6"/><ellipse class="rav-hand" cx="39" cy="30" rx="2.1" ry="2.7"/></g></g></g></svg>`;
  ...
  function scheduleBlink(){
    clearTimeout(blinkTimer);blinkTimer=setTimeout(()=>{const d=document.getElementById('rAvatarFrame');if(d&&!d.classList.contains('gaze-closed')&&!d.classList.contains('micro-close_eyes')){d.classList.add('blink');setTimeout(()=>d.classList.remove('blink'),150)}scheduleBlink()},2800+Math.random()*3600);
  }
  function microDuration(m){return m==='turn'?1250:m==='lean_in'||m==='pull_back'?1000:900}
  function playMicro(d,m){
    if(!m||m==='none')return;clearTimeout(microTimer);const cls='micro-'+m;d.classList.remove(cls);void d.offsetWidth;d.classList.add(cls);microTimer=setTimeout(()=>d.classList.remove(cls),microDuration(m));
  }
  function apply(raw,{active=true}={}){
    install();const a=norm(raw),d=document.getElementById('rAvatarFrame'),sameStamp=!!a.updatedAt&&a.updatedAt===lastPlayedStamp;
    current=a;localStorage.setItem(K.body,JSON.stringify(a));
    ...
    if(!sameStamp&&a.micro!=='none'){playMicro(d,a.micro);lastPlayedStamp=a.updatedAt}
  ...
  function legacyStage(text){
    ...
    const re=/\*?\s*[（(]([^）)]{1,180})[）)]\s*\*?/g;let clean=String(text||''),m,joined='',matched=false;
    ...
    const a={...def,updatedAt:Date.now()};
  ...
  install();const boot=safe(localStorage.getItem(K.body),def);lastPlayedStamp=Number(boot?.updatedAt||0);apply(boot,{active:false});
  const baseAdd=window.addMessage;window.addMessage=addMessage=async function(role,content,meta=''){if(role==='assistant'){let x=protocolExtract(content);if(!x.action){const y=legacyStage(x.text);x={text:y.text,action:y.action}}if(x.action)apply(x.action);content=x.text}return baseAdd(role,content,meta)};
