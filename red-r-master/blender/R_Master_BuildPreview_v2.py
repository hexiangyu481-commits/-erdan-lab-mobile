"""R Master v2 — clean body geometry diagnostic.

Builds on v1's measured mechanical proportion preview, then removes clothing-mask /
broken SurfaceDeform interference from the duplicated body mesh only and lowers
preview subdivision cost for faster cloud renders.

Input Mona source is never overwritten. No final Rest Pose bake. No VRM export.
"""
...
OUTPUT_COLLECTION = "R_MASTER_ALIGN_V2_PREVIEW"
OUTPUT_ARMATURE = "R_Master_Align_v2_PREVIEW"
...
CLEAN_DISABLE_TYPES = {"MASK", "SURFACE_DEFORM", "CLOTH"}
CLEAN_DISABLE_NAME_FRAGMENTS = ("mask", "cloth")
...
def parse_args():
    ...
    if not out_dir:
        ...
        out_dir = os.path.join(base, "R_Master_v2_Output")
    return os.path.abspath(out_dir)


def fail(msg):
    raise RuntimeError("R Master v2: " + msg)
...
def remove_previous_preview_collections():
    for name in ["R_MASTER_ALIGN_V0_PREVIEW", "R_MASTER_ALIGN_V1_PREVIEW", OUTPUT_COLLECTION]:
    ...
    for bone_name in sorted(CONSTRAINT_NEUTRALIZE_BONES):
        ...
        for c in pb.constraints:
            ...
            changed.setdefault(bone_name, []).append({
                "name": c.name,
                "type": c.type,
                "old_influence": float(c.influence),
            })
            c.influence = 0.0
    bpy.context.view_layer.update()
    return changed
...
def apply_hip_width_preview(rig):
    ...
    return {"before": before, "target": TARGET["hip_width_m"], "after": hip_width(rig)}


def find_preview_body(mesh_pairs):
    for src, dup in mesh_pairs:
        if src.name == SOURCE_BODY_MESH or src.data.name == SOURCE_BODY_MESH:
            return src, dup
    fail("Mona_Main duplicate not found")


def clean_body_modifiers(body):
    audit = []
    disabled = []
    lowered = []
    ...
    for mod in body.modifiers:
        row = {
            "name": mod.name,
            "type": mod.type,
            "show_viewport_before": bool(mod.show_viewport),
            "show_render_before": bool(mod.show_render),
        }

        lname = mod.name.lower()
        disable = (
            mod.type in CLEAN_DISABLE_TYPES
            or any(fragment in lname for fragment in CLEAN_DISABLE_NAME_FRAGMENTS)
        )
        ...
        if disable:
            ...
            old_levels = int(mod.levels)
            old_render = int(mod.render_levels)
            mod.levels = min(mod.levels, 1)
            mod.render_levels = min(mod.render_levels, 1)
            lowered.append({
                "name": mod.name,
                "type": mod.type,
                "levels_before": old_levels,
                "render_levels_before": old_render,
                "levels_after": int(mod.levels),
                "render_levels_after": int(mod.render_levels),
            })
        ...
        row["show_viewport_after"] = bool(mod.show_viewport)
        row["show_render_after"] = bool(mod.show_render)
        audit.append(row)

    bpy.context.view_layer.update()
    return {"audit": audit, "disabled": disabled, "lowered_preview_cost": lowered}


def set_only_visible_mesh(target, mesh_pairs, source_meshes):
...
def ensure_camera():
    cam_data = bpy.data.cameras.get("R_Master_v2_Camera_DATA") or bpy.data.cameras.new("R_Master_v2_Camera_DATA")
    cam = bpy.data.objects.get("R_Master_v2_Camera")
    if not cam:
        cam = bpy.data.objects.new("R_Master_v2_Camera", cam_data)
...
def setup_workbench(res_x=720, res_y=960):
...
def render_full_views(out_dir, body):
    ...
    for name, pos in views.items():
        cam.location = pos
        look_at(cam, target)
        path = os.path.join(out_dir, f"R_Master_v2_clean_body_{name}.png")
...
def render_pelvis_closeup(out_dir, body, rig):
    scene = setup_workbench(res_x=900, res_y=700)
    cam = ensure_camera()
    mn, mx = world_bounds(body)
    l = rig.pose.bones["UpperLeg.L"]
    r = rig.pose.bones["UpperLeg.R"]
    hip_center = Vector(((l.head.x + r.head.x) * 0.5, (l.head.y + r.head.y) * 0.5, (l.head.z + r.head.z) * 0.5))

    body_width = max(0.25, mx.x - mn.x)
    body_depth = max(0.20, mx.y - mn.y)
    dist = max(body_width, body_depth) * 5.0
    cam.data.ortho_scale = max(0.55, (mx.z - mn.z) * 0.42)

    outputs = {}
    views = {
        "front": Vector((hip_center.x, hip_center.y - dist, hip_center.z + 0.02)),
        "side": Vector((hip_center.x + dist, hip_center.y, hip_center.z + 0.02)),
    }

    for name, pos in views.items():
        cam.location = pos
        look_at(cam, hip_center)
        path = os.path.join(out_dir, f"R_Master_v2_pelvis_{name}.png")
        scene.render.filepath = path
        bpy.ops.render.render(write_still=True)
        outputs[name] = path
...
def mark_preview(rig):
    rig["red_master_stage"] = "R_Master_CleanBodyDiagnostic_v2"
    rig["red_master_restpose_baked"] = False
    rig["red_master_final_vrm"] = False
    rig["red_master_notes"] = (
        "Diagnostic duplicate only. Clothing masks, SurfaceDeform and cloth modifiers on the duplicated Mona_Main are disabled for clean structural inspection."
    )
...
def main():
    ...
    local_scales = apply_local_scales(rig)
    ...
    source_body, body = find_preview_body(mesh_pairs)
    modifier_clean = clean_body_modifiers(body)
    set_only_visible_mesh(body, mesh_pairs, source_meshes)
    mark_preview(rig)

    if bpy.context.mode != "OBJECT":
        bpy.ops.object.mode_set(mode="OBJECT")

    full_renders, bounds = render_full_views(out_dir, body)
    pelvis_renders = render_pelvis_closeup(out_dir, body, rig)
    ...
    report = {
        "ok": True,
        "stage": "R_Master_CleanBodyDiagnostic_v2",
        "source_file": os.path.abspath(bpy.data.filepath),
        "source_armature": SOURCE_ARMATURE,
        "source_body_mesh": source_body.name,
        "output_armature": OUTPUT_ARMATURE,
        "preview_body_mesh": body.name,
        "bone_count": bone_count,
        "duplicated_mesh_count": len(mesh_pairs),
        "constraints_neutralized": constraints_neutralized,
        "local_scales_applied": local_scales,
        "segment_validation": ratios,
        "hip_width": hip,
        "body_modifier_cleaning": modifier_clean,
        "rest_pose_baked": False,
        "adult_structure_independently_edited": False,
        "bounds": bounds,
        "renders": {
            "full": full_renders,
            "pelvis": pelvis_renders,
        },
        "acceptance": acceptance,
        "hard_stops": [
            "Do not use v2 as final VRM",
            "Do not bake rest pose yet",
            "Do not infer adult-anatomy correctness from this preview alone",
            "Do not delete/rename Mona adult anatomy/oral/face/finger/IK/control bones",
        ],
    }

    report_path = os.path.join(out_dir, "R_Master_v2_report.json")
    with open(report_path, "w", encoding="utf-8") as f:
        json.dump(report, f, ensure_ascii=False, indent=2)

    out_blend = os.path.join(out_dir, "R_Master_Align_v2_PREVIEW.blend")
    bpy.ops.wm.save_as_mainfile(filepath=out_blend, check_existing=False)

    print("[R Master v2] BUILD_OK")
    print("[R Master v2] mechanical_preview_pass:", acceptance["mechanical_preview_pass"])
    print("[R Master v2] disabled body modifiers:", modifier_clean["disabled"])
    print("[R Master v2] lowered preview modifiers:", modifier_clean["lowered_preview_cost"])
    print("[R Master v2] blend:", out_blend)
    print("[R Master v2] report:", report_path)
