...
function norm(a,{stamp=false}={}){if(!a||typeof a!=='object')return null;return {pose:poses.has(String(a.pose))?String(a.pose):def.pose,gaze:gazes.has(String(a.gaze))?String(a.gaze):def.gaze,expression:expressions.has(String(a.expression))?String(a.expression):def.expression,hand:hands.has(String(a.hand))?String(a.hand):def.hand,proximity:clamp(a.proximity,0,1,def.proximity),motion:motions.has(String(a.motion))?String(a.motion):def.motion,micro:micros.has(String(a.micro))?String(a.micro):def.micro,durationMs:Math.round(clamp(a.durationMs??a.duration_ms,600,9000,def.durationMs)),updatedAt:stamp?Date.now():Number(a.updatedAt||0)}}
function stable(a){if(!a)return'';const x=norm(a);return JSON.stringify({pose:x.pose,gaze:x.gaze,expression:x.expression,hand:x.hand,proximity:x.proximity,motion:x.motion,micro:x.micro,durationMs:x.durationMs})}
