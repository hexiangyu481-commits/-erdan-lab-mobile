...
function trimState(s){s=ensureState(s);s.recent=s.recent.slice(-30);s.outbox=s.outbox.slice(-50);s.privateThoughts=s.privateThoughts.slice(-20);s.ideas=s.ideas.slice(-20);s.unfinished=s.unfinished.slice(-16);s.rules=s.rules.slice(-40);s.surfHistory=s.surfHistory.slice(-12);s.identityContext=String(s.identityContext||"").slice(0,28000);return s}
...
function pushReply(s,jobId,text,usage,source,extra={}){const msg={id:crypto.randomUUID(),role:"assistant",content:String(text||"").trim(),createdAt:now(),source,usage,replyTo:jobId,...extra};s.outbox.push(msg);appendRecent(s,"assistant",msg.content);return msg}
function completedMessageInState(s,id){return (s.outbox||[]).find(x=>String(x?.replyTo||"")===id)||null}
function completedInState(s,id){return !!completedMessageInState(s,id)}
...
function intimacyLens(s,job){const hour=Number(job.clientLocalHour),night=Number.isFinite(hour)&&(hour>=23||hour<5),desire=clamp(s.adultState?.desire,0,100,18);return `你和用户都是成年人。${night?"现在是用户当地深夜，这只是一点更私密、更放松的氛围倾向，不是强制你变色情。":""}当前成人亲密倾向只是你的潜台词（约 ${desire}/100），不是要向用户汇报的数值。若用户主动发来的内容和上下文已经明确进入成年人、自愿、私密的亲密/色情语境，你可以比普通聊天更直接、更身体化、更色情地回应，不必装成图像审核员；但普通照片不要硬色情化。若画面中人物年龄不明确、可能未成年，或缺乏明确成年语境，不做性化描述。`}
...
async function callChat(env,{model,messages,temperature=.9}){if(!env.OPENROUTER_API_KEY)throw new Error("OPENROUTER_API_KEY missing");const body={model:model||"qwen/qwen3.8-27b",stream:false,temperature,usage:{include:true},provider:{data_collection:"deny"},messages};if(body.model==="qwen/qwen3.8-flash")body.reasoning={effort:"low",exclude:true};const r=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${env.OPENROUTER_API_KEY}`,"content-type":"application/json","HTTP-Referer":env.ALLOWED_ORIGIN||"https://hexiangyu481-commits.github.io","X-Title":"RED A8 Mind"},body:JSON.stringify(body)});const data=await r.json().catch(()=>({}));if(!r.ok)throw new Error(data?.error?.message||`OpenRouter ${r.status}`);const text=String(data?.choices?.[0]?.message?.content||"").trim();if(!text)throw new Error("OpenRouter returned empty content");return {text,usage:data?.usage||null}}
async function transcribe(env,audio){if(!env.OPENROUTER_API_KEY)throw new Error("OPENROUTER_API_KEY missing");const r=await fetch("https://openrouter.ai/api/v1/audio/transcriptions",{method:"POST",headers:{Authorization:`Bearer ${env.OPENROUTER_API_KEY}`,"content-type":"application/json","HTTP-Referer":env.ALLOWED_ORIGIN||"https://hexiangyu481-commits.github.io","X-Title":"RED A8 Mind"},body:JSON.stringify({model:"openai/whisper-large-v3",input_audio:{data:audio.data,format:audio.format}})});const data=await r.json().catch(()=>({}));if(!r.ok)throw new Error(data?.error?.message||`STT ${r.status}`);const text=String(data?.text||"").trim();if(!text)throw new Error("语音没有识别出正文");return {text,usage:data?.usage||null}}
...
function validateJob(raw){
  ...
  if(kind==="image"){const xs=Array.isArray(raw?.assets)?raw.assets.slice(0,MAX_IMAGES):[];if(!xs.length)throw new Error("image_required");let total=0;job.assets=xs.map((x,i)=>{const data=String(x?.data||"");total+=data.length;return {data,mime:String(x?.mime||"image/jpeg").slice(0,80),name:String(x?.name||`image-${i+1}.jpg`).slice(0,160)}});if(total>MAX_B64)throw new Error("media_too_large")}
  else{const a=raw?.audio||{},data=String(a.data||"");if(!data)throw new Error("audio_required");if(data.length>MAX_B64)throw new Error("media_too_large");job.audio={data,format:String(a.format||"mp4").replace(/[^a-z0-9]/gi,"").toLowerCase().slice(0,12)||"mp4",mime:String(a.mime||"audio/mp4").slice(0,80),durationMs:clamp(a.durationMs,0,60000,0)}}
  return job;
}

async function processJob(env,job,{persist=true}={}){
  let s=await getState(env);if(completedInState(s,job.jobId)){const old=completedMessageInState(s,job.jobId);return {completed:true,transcript:old?.transcript||undefined,state:publicState(s)}}
  const identity=String(job.identityContext||s.identityContext||"").slice(0,10000),system=[identity,continuity(s),voiceGuard(),intimacyLens(s,job),protocol()].filter(Boolean).join("\n\n"),recent=compactTextMessages(job.messages);let rawText,usage,transcript="",userRecord="";
  if(job.kind==="image"){
    const content=[{type:"text",text:job.text||((job.assets||[]).length>1?"我刚给你发了几张照片。先真正看完，再直接以你自己的身份回应我。":"我刚给你发了张照片。先真正看，再直接回应我。")}];for(const a of job.assets||[])content.push({type:"image_url",image_url:{url:`data:${a.mime||"image/jpeg"};base64,${a.data}`}});
  ...
  const parsed=parseOutput(rawText);if(!parsed.text)throw new Error("empty_media_reply");applyPatch(s,parsed.state);appendRecent(s,"user",userRecord);pushReply(s,job.jobId,parsed.text,usage,job.kind==="image"?"media_image":"media_audio",{mediaKind:job.kind,transcript:transcript||undefined});
...
export async function mediaStatus(env,id){
  ...
  const s=await getState(env),done=completedMessageInState(s,id);if(done)return {ok:true,completed:true,queued:false,replyId:done.id,transcript:done.transcript||undefined,mediaKind:done.mediaKind||undefined};
...
export async function acceptMedia(req,env,ctx,onComplete=null){
  const raw=await req.json(),job=validateJob(raw),key=MEDIA_PREFIX+job.jobId,status=await mediaStatus(env,job.jobId);if(status.completed)return {status:200,data:{accepted:true,duplicate:true,jobId:job.jobId,...status}};if(status.queued)return {status:202,data:{accepted:true,completed:false,duplicate:true,jobId:job.jobId,queued:true}};
  try{await env.RED_STATE.put(key,JSON.stringify(job),{expirationTtl:MEDIA_TTL});ctx.waitUntil(processStoredMedia(env,key).then(async r=>{if(r?.completed&&onComplete)await onComplete(r)}).catch(e=>console.error("RED media background failed",String(e?.message||e))));return {status:202,data:{accepted:true,completed:false,jobId:job.jobId,queued:true,kind:job.kind}}}
  catch(e){if(!isKVWriteLimitError(e))throw e;const r=await processJob(env,job,{persist:false});return {status:200,data:{accepted:true,completed:true,degraded:true,degradedReason:"kv_write_limit",...r}}}
}
export async function resumeOneMedia(env,onComplete=null){
  const list=await env.RED_STATE.list({prefix:MEDIA_PREFIX,limit:2}),key=list?.keys?.[0]?.name;if(!key)return {skipped:"none"};
