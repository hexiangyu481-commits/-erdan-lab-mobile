...
const AURA_MODES=new Set(["calm","happy","teasing","sad","hesitant"]);
...
function ensureAuraState(s){
  if(!s.auraState||typeof s.auraState!=="object")s.auraState={mode:"calm",updatedAt:0};
  if(!AURA_MODES.has(String(s.auraState.mode)))s.auraState.mode="calm";
  s.auraState.updatedAt=Number(s.auraState.updatedAt||0);
...
function applyAuraDecision(s,x,{shiftLimit=25}={}){
  ...
  const aura=String(x?.aura||"").toLowerCase();if(AURA_MODES.has(aura)){s.auraState={mode:aura,updatedAt:now()};}
  let peak=false;
...
async function wake(env,force=false){
  ...
  if(!Array.isArray(s.wakeTrace))s.wakeTrace=[];s.wakeTrace.push({at:now(),action:decision.action,mood:String(s.mood||"平静").slice(0,120),aura:s.auraState.mode,nextMinutes:nextMin,sent,desire:s.adultState.desire,desireMode:s.adultState.mode,peak:auraResult.peak});s.wakeTrace=s.wakeTrace.slice(-30);
